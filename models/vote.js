...
class Vote {
    ...
    static async submittVote({ washroomId, username, voteType }) {
        ...
        if (isThereVoteAlready) {
            ...
            const res = await this.updateVote({washroomId, currentVote : isThereVoteAlready.upvote})
            return res.rows
        }
    ...
    static async updateVote({ washroomId, currentVote }) {
        const newVote = currentVote === 1 ? 0 : 1;
        const res = await db.query(`UPDATE votes SET upvote = $1 WHERE post_id = $2 
                                    RETURNING id, post_id, user_id, upvote`, [newVote, washroomId])
        return res.rows
    }
